export function BOTTLE_FACTORY(container) {
  ...
  const logger = container[BOTTLE_NAMES.LIB_LOGGER].getContextualLogger('service.plaid_data');
  ...
  const SERVICE = {
    ...
    writePlaidData: async ({ userId, plaidAccountData }) => {
      ...
      try {
        await awsLib.s3PutObject(userDataBucket, userPlaidDataKey, JSON.stringify(plaidAccountData));
