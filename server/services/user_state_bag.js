...
export function BOTTLE_FACTORY(container) {
  ...
  const logger = container[BOTTLE_NAMES.LIB_LOGGER].getContextualLogger('service.user_state_bag');
  ...
  const SERVICE = {
    ...
    verifyPreviousStateEqualsCurrentState: (previousState, currentState) => {
      try {
        assert.deepStrictEqual(previousState, currentState);
      } catch (e) {
    ...
    writeUserState: async (userId, userState) => {
      ...
      try {
        await awsLib.s3PutObject(userDataBucket, userStateBagKey, JSON.stringify(userState));
    ...
    readUserState: async userId => {
      ...
      try {
        const response = await awsLib.s3GetObject(userDataBucket, userStateBagKey);
