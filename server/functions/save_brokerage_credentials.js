...
export const handler = async function(event, context, container, callback) {
  ...
  const logger = container[BOTTLE_NAMES.LIB_LOGGER].getContextualLogger('save_brokerage_credentials.handler');
  ...
  const results = await helperLib.executeAllPromises([encryptedS3UploadPromise, obfuscatedS3UploadPromise]);
