...
export const handler = async function(event, context, container, callback) {
  ...
  const logger = container[BOTTLE_NAMES.LIB_LOGGER].getContextualLogger('save_user_email_identity_link.handler');
  ...
  const createLinkPromise = awsLib.s3PutObject(USER_DATA_BUCKET, userEmailIdentityLink, '');
