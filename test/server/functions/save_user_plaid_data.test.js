  ...
  describe('handler', () => {
    ...
    describe('when there is some error thrown during execution', () => {
      ...
      it('should log and invoke a failure callback', async () => {
        const result = await util.promisify(saveUserPlaidDataService.handler)(event, context);

        expect(result).to.deep.equal(
          container[BOTTLE_NAMES.LIB_RESPONSE].failure({
    ...
    describe('when it succeeds', () => {
      ...
      it('should work correctly', async () => {
        ...
        expect(result).to.deep.equal(container[BOTTLE_NAMES.LIB_RESPONSE].success(plaidDataWriteResponse));
        ...
        expect(
          container[BOTTLE_NAMES.SERVICE_LAMBDA_ENVIRONMENT_HELPER].getCognitoIdentityId.lastCall.args
        ).to.deep.equal([event]);
