  ...
  describe('getPlaidAccessToken', () => {
    describe('when the public token exchange fails', () => {
      ...
      it('should throw and log', async () => {
        const err = await plaidAuthenticatorService
          .getPlaidAccessToken({
            plaidAccountId,
            plaidPublicToken,
          })
    ...
    describe('when everything succeeds', () => {
      ...
      it('should return the tokens', async () => {
        ...
        expect(mockExternPlaid.exchangePublicToken.lastCall.args).to.deep.equal([plaidPublicToken]);
      });
    });
  });
  ...
  describe('getPlaidInstitutionName', () => {
    ...
    describe('when everything succeeds and we got a name', () => {
      ...
      it('should return the institution name', async () => {
        ...
        expect(mockExternPlaid.getAuth.lastCall.args).to.deep.equal([plaidAccessToken]);

        expect(mockExternPlaid.getInstitutionById.lastCall.args).to.deep.equal([authResponse.item.institution_id]);
      });
    });
  });
});
